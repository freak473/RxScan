...
**Version:** 0.4.2 (final — requirements closed)
**Owners:** Maalik, [friend]
**Status:** Requirements closed — build (pair with `RxScan-v2-design-v3.html` and `rxscan-tech-design-v0_2_1.md`)
**Last updated:** 12 July 2026

> **Changes in v0.4.2:** Added the **notification-permission strategy** (§6 step 4, §8): POST_NOTIFICATIONS is a runtime permission on Android 13+ and is requested with a primer immediately after OTP at the save moment (visible purpose; the system dialog is a two-refusal resource, never spent at app launch); exact-alarm access is the separate, skippable second step (a Settings deep-link, not a dialog); battery-optimization exemption is asked contextually later; denial still saves and schedules, with a persistent recovery banner on Today. Design updated with the priming screen.
>
...
**Separability principle:** the engine, eval set (opt-in corrections only), and API business are structured to be transferable *without* the consumer data. In v0.4 the user's prescription record is stored server-side, but it is **encrypted and keyed to a pseudonymous `userId`, held in a store separate from the engine/eval-set** (§7, §8). Under envelope encryption it remains a distinct asset that can be carved out or deleted on a change of control; under the E2E/zero-knowledge option (§13) it is ciphertext an acquirer cannot read. Either way, the transferable assets are the engine, eval set, and API — not the consumer record. Under the DPDP Act, consent is purpose-specific and does not automatically survive a change of control for new purposes, so keeping the consumer store separate and encrypted is what makes a clean exit possible. We never soften consumer privacy promises to make a future sale easier; the sellable asset is the engine, not the users.
...
1. **Capture** — Open app → camera with prescription framing guide (edge detection, glare warning). Option to import from gallery/WhatsApp. **Offline capture:** if there's no connectivity, the photo is queued and extracted when the device reconnects; the user is notified when their check-list is ready.
2. **Upload & extract** — Image sent to backend. Async processing with progress state (target p50 < 8s).
3. **Verify (hard gate)** — Extraction results shown as editable cards, one per medicine. **The user must actively confirm each medicine before anything can be scheduled — this screen cannot be skipped or bulk-accepted blindly.**
...
4. **Sign in + save + schedule** — On first save, the user creates/enters their account via **phone + OTP** (the only personal detail we take is the phone number). The scan and verify steps above run before login so the zero-friction promise at the clinic holds; login is prompted at the moment of saving. The confirmed schedule is then **persisted server-side, encrypted, under the user's `userId`** and cached on-device for offline reminders. The app asks for their meal times once (breakfast/lunch/dinner + **bedtime** slot for HS medicines; defaults, editable) → local notifications scheduled for every dose until course end. **AC/PC offsets:** before food fires 30 min before the meal time, after food 30 min after; both user-adjustable. **Permissions at the save moment (v0.4.2):** immediately after OTP success — with 30 reminders one tap away, so the ask is legible — a one-screen primer precedes the Android 13+ **POST_NOTIFICATIONS** system dialog (the dialog tolerates ~two refusals before Android permanently silences it, so it is never triggered cold at app launch). **Exact-alarm access** (Android 14+, a Settings toggle via deep-link, not a dialog) is offered as a separate, skippable second step; declining falls back to WorkManager windows with an in-app accuracy note (§8). **Battery-optimization exemption** is requested contextually later (e.g., after a late-fired reminder), never stacked at onboarding. On notification denial, everything is still saved and scheduled, and Today shows a persistent "reminders are silenced" banner deep-linking to Settings. Android ≤12 auto-grants; the step is API-gated.
5. **Daily use** — Notification with actions: **Taken / Snooze 30m / Skip**. Responses logged to adherence history (device cache → synced to the server-side record). **Notification privacy:** lock-screen text is discreet by default ("Night medicines · 2 due"), never naming drugs; showing names is an explicit setting. Multiple doses at one time are grouped into a single notification.
6. **Course end** — "Your Augmentin course ends tomorrow" notification; reminders auto-stop after the last dose. From the progress view, the user can **send their adherence report to their doctor** via the native share sheet (device-to-doctor; our servers never relay it).
...
- **Formulary constraint is the biggest accuracy lever.** Fuzzy-matching LLM output against a real Indian drug database converts near-misses into exact matches. Strength checks **flag** anomalies for user re-verification; they do **not** auto-correct or suggest (a correction would edge toward substitution/advice → SaMD risk, see §3.1).
...
**Access model (v0.4 — two planes):**

- **Engine plane (client-keyed, identity-free):** the extraction API (`/v1/extractions`, eval-set retention, corrections, formulary) is authenticated with a **client key** (our app ships with an app-signed key; partner keys are issued manually) and metered per extraction. **No user identity is attached to extraction calls** — client keys identify the *application*, not the person. This keeps the engine a clean, separable, sellable asset (§2.5) and lets partners use it without touching our consumer accounts. Eval-set retention (opt-in) is keyed to a **pseudonymous retention token**, never to the user account.
- **Consumer plane (phone-OTP, `userId`):** the app authenticates the *user* via phone + OTP, receiving a session token. The confirmed prescription record is stored under a pseudonymous `userId` via authenticated endpoints, **encrypted at rest**. The phone number is the only PII, stored encrypted with a keyed-HMAC blind index for login lookup. Delete/export operate by `userId`.

The two planes are deliberately separate: the engine never learns who the user is; the consumer store never mixes into the engine/eval-set data.

## 8. System architecture (v1)

- **Backend:** Spring Boot service. **Engine plane:** extraction API + formulary service, PostgreSQL, S3 for *transient* image processing, async worker queue for extraction jobs; vision LLM called via provider API under a Data Processor agreement; API gateway with client keys, per-key metering/rate limits, versioned contract, sandbox environment (§2.5). **Consumer plane:** phone-OTP auth service + an **encrypted prescription store** (per-user envelope encryption under a KMS/HSM master key), holding the confirmed record keyed to `userId`; the only PII is the phone number, stored encrypted with a keyed-HMAC blind index.
- **App:** Android (Kotlin, native) — camera capture, verification UI, local notification scheduling (AlarmManager/WorkManager for exact-time delivery incl. Doze handling), **local cache of the schedule + adherence log (Room)** so reminders fire offline. The **server is the system of record**; the device cache is populated from it and queues writes (new schedules, adherence events) for push on reconnect. **Auth: phone + OTP** (the sole personal detail is the phone number). The scan/verify steps run before login; saving/scheduling requires the account. The account is also the rail for Pro/caregiver mode in v2 and makes delete/export trivially enforceable.
- **Notifications are local, not push** — reminders must fire without connectivity and without alarm-clock hostility (actionable notification, not alarm).
  - **Notification permission (Android 13+):** POST_NOTIFICATIONS is runtime and existential for this product; requested with a primer at the save moment per §6 step 4, with the two-refusal budget respected, a denial-state banner on Today, and a high-importance "Dose reminders" notification channel created at first schedule.
...
| **1. Extraction API + consumer backend** | Spring Boot service, formulary DB + fuzzy match, strength anomaly flags, confidence scoring, client-key auth + metering, consented extraction logging, Data Processor agreement with LLM provider; **phone-OTP auth service; encrypted prescription store (envelope encryption + KMS/HSM); delete/export by userId** | 4 wks |
| **2. Android core** | Capture (incl. offline queue) → verify (hard gate, flag inputs, ask-your-doctor share) → **phone-OTP sign-in** → schedule (AC/PC offsets, bedtime slot, ongoing durations) → notify (exact-alarm onboarding, discreet lock screen) → adherence log; **server-backed store with on-device cache for offline firing**; purpose-specific consent; delete/export | 4 wks |
...
5. ~~Account required in v1?~~ **Resolved (v0.4): yes.** Phone-OTP account; the confirmed prescription record is stored server-side, encrypted, keyed to a `userId`; the only PII retained is the phone number. Scan/verify run before login to preserve zero-friction; saving requires the account.
...
13. ~~**Account mandatory before scan, or only at save?**~~ **Resolved (v0.4.1): at save.** Scan/verify run pre-login; phone-OTP is prompted when the user taps "Set my reminders" (after meal times), and on success the reminders schedule immediately — the tightest possible ask-reward loop. Rationale: (a) the correct extraction is the product's proof, and the ask lands *after* the proof; (b) the ask carries a visible, DPDP-clean purpose at that moment ("so your reminders survive a phone change") instead of a purpose-free upfront wall; (c) the doctor-recommendation loop — an app that demands a number before helping reads as lead capture; (d) OTP SMS cost is paid only for activated users; (e) honest-failure users leave nothing behind (no orphan phone numbers = smaller breach surface). Pre-login scans are metered per device (client key + device attestation); the per-user cap applies after sign-in. Returning users hold a session and never see the wall again.
